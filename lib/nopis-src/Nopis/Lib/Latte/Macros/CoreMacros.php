...
use Nopis\Lib\Latte,
    Nopis\Lib\Latte\CompileException,
...
class CoreMacros extends MacroSet
{
    ...
    {
        $code = $writer->write('$_b->templates[%var]->renderChildTemplate(%node.word, %node.array? + $template->getParameters())', $this->getCompiler()->getTemplateId());
    ...
    public function macroEndForeach(MacroNode $node, PhpWriter $writer)
    {
        if ($node->modifiers !== '|noiterator' && preg_match('#\W(\$iterator|include|require|get_defined_vars)\W#', $this->getCompiler()->expandTokens($node->content))) {
            $node->openingCode = '<?php $iterations = 0; foreach ($iterator = $_l->its[] = new Nopis\Lib\Latte\Runtime\CachingIterator('
    ...
    {
        return $writer->write('echo Nopis\Lib\Latte\Runtime\Filters::htmlAttributes(%node.array)');
    }
    ...
    public function macroContentType(MacroNode $node, PhpWriter $writer)
    {
        if (strpos($node->args, 'xhtml') !== FALSE) {
            $this->getCompiler()->setContentType(Compiler::CONTENT_XHTML);
        } elseif (strpos($node->args, 'html') !== FALSE) {
            $this->getCompiler()->setContentType(Compiler::CONTENT_HTML);
        } elseif (strpos($node->args, 'xml') !== FALSE) {
            $this->getCompiler()->setContentType(Compiler::CONTENT_XML);
        } elseif (strpos($node->args, 'javascript') !== FALSE) {
            $this->getCompiler()->setContentType(Compiler::CONTENT_JS);
        } elseif (strpos($node->args, 'css') !== FALSE) {
            $this->getCompiler()->setContentType(Compiler::CONTENT_CSS);
        } elseif (strpos($node->args, 'calendar') !== FALSE) {
            $this->getCompiler()->setContentType(Compiler::CONTENT_ICAL);
        } else {
            $this->getCompiler()->setContentType(Compiler::CONTENT_TEXT);
        }
