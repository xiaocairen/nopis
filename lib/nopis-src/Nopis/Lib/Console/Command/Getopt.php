...
namespace Nopis\Lib\Console\Command;
...
class Getopt implements \Countable, \ArrayAccess, \IteratorAggregate
{
    ...
    public function __construct($options = null,
                                $defaultType = Getopt::NO_ARGUMENT)
    {
    ...
    {
        if ((null === $option1->short() && null === $option2->short()) || (null === $option1->long() && null === $option2->long())) {
            return false;
        }
        return ((($option1->short() === $option2->short()) && ($option1->long() !== $option2->long())) || (($option1->short() !== $option2->short()) && ($option1->long() === $option2->long())));
    }
    ...
    public function parse($arguments = null)
    {
        ...
        $parser->parse($arguments, $this);
        $this->options  = $parser->getOptions();
        $this->operands = $parser->getOperands();
    }

    /**
     * Return all options of current Command
     *
     * @param int $start
     * @param int $offset
     * @return mixed
     */
    public function getCmdOpt($start = 0, $offset = null)
    {
        if (null === $offset) {
            $offset = count($this->cmdOpt);
        }
        return array_slice($this->cmdOpt, $start, $offset);
    }

    /**
     * Save the option of current Command for get
     *
     * @param type $cmd
     */
    public function setCmdOpt($cmd)
    {
        $this->cmdOpt[] = $cmd;
    }
     ...
     *
     * @param string $optCommand
     * @return Option
     */
    public function getOptionList($optCommand)
    {
        foreach ($this->optionList as $option) {
            if ($option->matches($optCommand)) {
                return $option;
            }
        }
    }
    ...
    public function getIterator()
    {
        ...
        foreach ($this->options as $name => $value) {
            $keep = true;
            foreach ($this->optionList as $option) {
                if ($option->long() == $name && null !== $option->short()) {
                    $keep = false;
                }
            }
            if ($keep) {
