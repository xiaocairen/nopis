...
use Nopis\Lib\Latte,
	...
	Nopis\Lib\Latte\RuntimeException;
...
class BlockMacros extends MacroSet
{
	...
	public function macroInclude(MacroNode $node, PhpWriter $writer)
	{
		...
		if (isset($this->namedBlocks[$destination]) && !$parent) {
			...
			$cmd = 'Nopis\Lib\Latte\Macros\BlockMacros::callBlock' . ($parent ? 'Parent' : '') . "(\$_b, $name, %node.array? + " . ($parent ? 'get_defined_vars' : '$template->getParameters') . '())';
		}
