...
class PhpWriter extends LatteObject
{
    ...
    public function write($mask)
    {
        ...
        $code = preg_replace_callback('#([,+]\s*)?%(node_|\d+_|)(word|var|raw|array|args)(\?)?(\s*\+\s*)?()#', function($m) use ($me, $word, & $args) {
    ...
    {
        return (is_numeric($s) || preg_match('#^\$|[\'"]|^true\z|^false\z|^null\z#i', $s)) ? $this->formatArgs(new MacroTokens($s)) : '"' . $s . '"';
    }
    ...
    {
        $res       = new MacroTokens;
        $tplParams = $this->compiler->getTemplateParams();
        $prevParam = null;
        $prevUnset = false;
        while ($tokens->nextToken()) {
            /** Start 添加使模版可以如{$user.name.firsts}这样的变量写法，并将其解析为数组或对象变量 */
            $curValue = $tokens->currentValue();
            if (0 === strpos($curValue, '$') && $tokens->isNext('.')) {
                $varName = substr($curValue, 1);
                if (isset($tplParams[$varName])) {
                    $prevParam = $tplParams[$varName];
                }
            } elseif ($prevParam !== null && $tokens->isCurrent('.')) {
                $tokens->unsetCurrentToken();
                $prevUnset = true;
            } elseif ($prevParam !== null && $prevUnset && $tokens->isCurrent(MacroTokens::T_SYMBOL)) {
                $prevUnset = false;
                if (is_object($prevParam)) {
                    $res->append('->' . $curValue);
                    $prevParam = $prevParam->$curValue;
                } else {
                    $res->append("['" . $curValue . "']");
                    $prevParam = $prevParam[$curValue];
                }
            ...
            if ($tokens->isCurrent(MacroTokens::T_WHITESPACE)) {
                ...
                if ($tokens->isCurrent(MacroTokens::T_SYMBOL)) {
                    if ($this->compiler && $tokens->isCurrent('escape')) {
                        ...
                        $res->prepend('Nopis\Lib\Latte\Runtime\Filters::safeUrl(');
                        $inside = TRUE;
                    } else {
    ...
    public function escapeFilter(MacroTokens $tokens)
    {
        ...
        switch ($this->compiler->getContentType()) {
                ...
                switch ($context[0]) {
                    ...
                    case Compiler::CONTEXT_UNQUOTED_ATTR:
                        if ($context[1] === Compiler::CONTENT_JS) {
                            $tokens->prepend('Nopis\Lib\Latte\Runtime\Filters::escapeJs(')->append(')');
                        } elseif ($context[1] === Compiler::CONTENT_CSS) {
                            $tokens->prepend('Nopis\Lib\Latte\Runtime\Filters::escapeCss(')->append(')');
                        }
                        $tokens->prepend('Nopis\Lib\Latte\Runtime\Filters::escapeHtml(')->append($context[0] === Compiler::CONTEXT_SINGLE_QUOTED_ATTR ? ', ENT_QUOTES)' : ', ENT_COMPAT)');
                        if ($context[0] === Compiler::CONTEXT_UNQUOTED_ATTR) {
                            $tokens->prepend("'\"' . ")->append(" . '\"'");
                        }
                        return $tokens;
                    case Compiler::CONTEXT_COMMENT:
                        return $tokens->prepend('Nopis\Lib\Latte\Runtime\Filters::escapeHtmlComment(')->append(')');
                    case Compiler::CONTENT_JS:
                    case Compiler::CONTENT_CSS:
                        return $tokens->prepend('Nopis\Lib\Latte\Runtime\Filters::escape' . ucfirst($context[0]) . '(')->append(')');
                    default:
                        return $tokens->prepend('Nopis\Lib\Latte\Runtime\Filters::escapeHtml(')->append(', ENT_NOQUOTES)');
                }
            ...
            case Compiler::CONTENT_XML:
                ...
                switch ($context[0]) {
                    case Compiler::CONTEXT_COMMENT:
                        return $tokens->prepend('Nopis\Lib\Latte\Runtime\Filters::escapeHtmlComment(')->append(')');
                    default:
                        $tokens->prepend('Nopis\Lib\Latte\Runtime\Filters::escapeXml(')->append(')');
                        if ($context[0] === Compiler::CONTEXT_UNQUOTED_ATTR) {
                            $tokens->prepend("'\"' . ")->append(" . '\"'");
                        }
                        return $tokens;
            ...
            case Compiler::CONTENT_ICAL:
                return $tokens->prepend('Nopis\Lib\Latte\Runtime\Filters::escape' . ucfirst($this->compiler->getContentType()) . '(')->append(')');
