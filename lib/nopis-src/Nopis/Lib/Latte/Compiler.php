...
class Compiler extends LatteObject
{
    ...
    /** @var string */
    private $userFuncPrefix = 'np_tpl_object_';
    ...
    protected function processText(Token $token)
    {
    ...
    protected function processMacroTag(Token $token)
    {
    ...
    protected function processHtmlTagBegin(Token $token)
    {
        if ($token->closing) {
            while ($this->htmlNode) {
            ...
            $this->htmlNode->isEmpty = in_array($this->contentType, array(self::CONTENT_HTML, self::CONTENT_XHTML), TRUE) && isset(Helpers::$emptyElements[strtolower($token->name)]);
            $this->htmlNode->offset  = strlen($this->output);
            $this->setContext(self::CONTEXT_UNQUOTED_ATTR);
        }
        $this->output .= $token->text;
    ...
    protected function processHtmlTagEnd(Token $token)
    {
        if ($token->text === '-->') {
            $this->output .= $token->text;
            $this->setContext(NULL);
        ...
        if ($htmlNode->closing) {
            $this->htmlNode = $this->htmlNode->parentNode;
        } elseif ((($lower = strtolower($htmlNode->name)) === 'script' || $lower === 'style') && (!isset($htmlNode->attrs['type']) || preg_match('#(java|j|ecma|live)script|css#i', $htmlNode->attrs['type']))
        ) {
            $this->setContext($lower === 'script' ? self::CONTENT_JS : self::CONTENT_CSS);
        }
    }

    protected function processHtmlAttribute(Token $token)
    {
        ...
        if (in_array($this->contentType, array(self::CONTENT_HTML, self::CONTENT_XHTML), TRUE)) {
            ...
            } elseif (in_array($lower, array('href', 'src', 'action', 'formaction'), TRUE) || ($lower === 'data' && strtolower($this->htmlNode->name) === 'object')
            ) {
    ...
    protected function processComment(Token $token)
    {
    ...
    public function closeMacro($name, $args = NULL, $modifiers = NULL, $isRightmost = FALSE, $nPrefix = NULL)
    {
        ...
        if (!$node || ($node->name !== $name && '' !== $name) || $modifiers || ($args && $node->args && strncmp("$node->args ", "$args ", strlen($args) + 1)) || $nPrefix !== $node->prefix
        ) {
    ...
    public function expandMacro($name, $args, $modifiers = NULL, $nPrefix = NULL)
    {
        ...
        foreach (array_reverse($this->macros[$name]) as $macro) {
            $node = new MacroNode($macro, $name, $args, $modifiers, $this->macroNode, $this->htmlNode, $nPrefix, $this->getFuncs());
     ...
     *
     * @return array  [object_varname, $object]
     *
     * @time 2015-08-06
     */
    ...
    {
        // 给对象变量名加上前缀并引用到源对象
        $newObjectName = $this->userFuncPrefix . $funcArr[0];
        $$newObjectName = & $funcArr[1];
        $this->userFuncs[$funcName] = [$newObjectName, $$newObjectName, $funcArr[2]];

        return [$newObjectName, $$newObjectName];
    }
    ...
    {
        return isset($this->userFuncs[$funcName]) ? $this->userFuncs[$funcName] : null;
    }
