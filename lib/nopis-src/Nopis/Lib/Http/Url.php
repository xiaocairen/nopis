 ...
 *   |      |      |        |      |    |             |
 * /--\   /--\ /------\ /-------\ /--\/--\/----------------------------\
 * http://john:x0y17575@Nopis.com:8080/zh_cn/manual.php?name=param#fragment  <-- absoluteUrl
 ...
 * - hostUrl:     http://user:password@Nopis.com:8080
 * - basePath:    /en/ (everything before relative URI not including the script name)
 * - baseUrl:     http://user:password@Nopis.com:8080/en/
...
class Url
{
    ...
    public function __construct($url = NULL)
    {
        if (is_string($url)) {
            ...
            if ($parts === FALSE) {
                throw new \InvalidArgumentException("Unsupported URI '$url'.");
            }
        ...
        if (preg_match('/^\d+\.\d+\.\d+\.\d+$/', $this->host)) {
            $this->subDomain = 'www';
            $this->topDomain = $this->host;
        } else {
            $h = explode('.', $this->host);
            $this->subDomain = implode('.', array_slice($h, 0, -2, true));
            $this->topDomain = implode('.', array_slice($h, -2));
            if ($this->topDomain == 'com.cn' || $this->topDomain == 'net.cn' || $this->topDomain == 'org.cn') {
                $this->subDomain = implode('.', array_slice($h, 0, -3, true));
                $this->topDomain = implode('.', array_slice($h, -3));
            }
        }
    }
    ...
    public static function unescape($s, $reserved = '%;/?:@&=+$,')
    {
        ...
        if ($reserved !== '') {
            $s = preg_replace_callback(
                    '#%(' . substr(chunk_split(bin2hex($reserved), 2, '|'), 0, -1) . ')#i', function($m) {
